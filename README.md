...
1. **نصب وابستگی‌ها**
   ...
   pnpm install
   ```
   در صورتی که پشت فایروال هستید قبل از اجرا از `pnpm run check:registry` برای اطمینان از دسترسی به رجیستری استفاده کنید.

2. **ساخت فایل پیکربندی**
   ```bash
   cp .env.example .env.local
   ```
   سپس مقادیر زیر را حداقل مقداردهی کنید:
   - `GOOGLE_CLIENT_ID` و `GOOGLE_CLIENT_SECRET` با مقادیری که از Google Cloud دریافت کرده‌اید.
   - `NEXTAUTH_SECRET` یک رشتهٔ تصادفی حداقل ۳۲ کاراکتری.
   - `ADMIN_EMAILS` با یک یا چند ایمیل Gmail که اجازهٔ ورود دارند (حداکثر سه ایمیل).
   - در صورت اجرای Postgres محلی (مثلاً از طریق Docker)، مقدار `DATABASE_URL` را مطابق میزبان/کاربر مورد نظر به‌روزرسانی کنید.

3. **اجرای توسعه‌ای**
   ```bash
   pnpm dev
   ```
   برنامه روی آدرس `http://localhost:3000` اجرا می‌شود. صفحهٔ اصلی (`/`) و مسیرهای `/fa`، `/en` و همچنین پنل مدیریتی (`/admin`) را بررسی کنید. برای ورود به پنل از مسیر `/login` استفاده کرده و با یکی از ایمیل‌های تعریف‌شده در Allowlist وارد شوید.

4. **ساخت و اجرای تولیدی در همان ماشین**
   ```bash
   pnpm build
   pnpm start
   ```
   دستور دوم خروجی Production را سرو می‌کند. اطمینان حاصل کنید که صفحات عمومی و مسیرهای مدیریتی بدون خطا باز می‌شوند و در لاگ سرور خطایی مشاهده نمی‌شود.
...
> **توجه:** در محیط تولید استفاده از `.env` (برای Docker) و در محیط توسعه استفاده از `.env.local` (برای اجرای مستقیم Next.js) توصیه می‌شود. مقادیر اصلی این دو فایل می‌تواند مشترک باشد اما از اشتراک‌گذاری فایل‌های حاوی اسرار در مخزن خودداری کنید.
